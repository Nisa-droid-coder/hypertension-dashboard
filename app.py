...
try:
    ...
    st.warning("SHAP not installed. Install it using: pip install shap")
...
if 'age_range' not in st.session_state:
    st.session_state.age_range = (18, 35)
if 'hypertension_filter' not in st.session_state:
    st.session_state.hypertension_filter = ["All"]
if 'bin_size' not in st.session_state:
    st.session_state.bin_size = 5
if 'chart_theme' not in st.session_state:
    st.session_state.chart_theme = "plotly_white"
if 'last_trained_age_range' not in st.session_state:
    st.session_state.last_trained_age_range = None
if 'shap_values' not in st.session_state:
    st.session_state.shap_values = {}
if 'shap_explainers' not in st.session_state:
...
def train_models_improved(df, age_range):
    ...
    try:
        ...
        df_young = df[(df['Age'] >= age_range[0]) & (df['Age'] <= age_range[1])].copy()
        
        if len(df_young) < 10:
            st.error(f"Only {len(df_young)} samples in age range {age_range[0]}-{age_range[1]}. Need at least 10 samples to train models.")
            return None
                ...
                try:
                    with warnings.catch_warnings():
                        ...
                        if name == "Logistic Regression":
                            ...
                            explainer = shap.LinearExplainer(model, curr_X_train, feature_names=feature_names)
                            shap_values = explainer.shap_values(curr_X_test)
                            shap_values_dict[name] = shap_values
                            shap_explainers_dict[name] = explainer
                        elif name == "Random Forest":
                            # Use TreeExplainer for Random Forest with proper handling
                            # Sample background data to speed up computation
                            background = shap.sample(curr_X_train, min(100, len(curr_X_train)))
                            explainer = shap.TreeExplainer(model, data=background, feature_perturbation="interventional")
                            shap_values = explainer.shap_values(curr_X_test)
                            # For binary classification, shap_values is a list [shap for class0, shap for class1]
                            if isinstance(shap_values, list):
                                shap_values_dict[name] = shap_values[1]  # Take class 1 (hypertension)
                            else:
                                shap_values_dict[name] = shap_values
                            shap_explainers_dict[name] = explainer
                            ...
                            if isinstance(shap_values, list):
                                shap_values_dict[name] = shap_values[1]
                            else:
                                shap_values_dict[name] = shap_values
                            shap_explainers_dict[name] = explainer
                except Exception as e:
                    st.warning(f"Could not compute SHAP values for {name}: {str(e)}")
        ...
        return {
            ...
            'samples_used': len(df_young),
            'shap_values': shap_values_dict,
            'shap_explainers': shap_explainers_dict
        }
...
def auto_train_if_needed():
    """Automatically retrain models if age range has changed"""
    if st.session_state.df is not None:
        current_age_range = st.session_state.age_range
        last_trained = st.session_state.last_trained_age_range
        
        # Check if we need to retrain
        need_retrain = (
            not st.session_state.models_trained or
            last_trained != current_age_range
        )
        
        if need_retrain:
            with st.spinner(f"🔄 Auto-training models for age range {current_age_range[0]}-{current_age_range[1]} years..."):
                result = train_models_improved(st.session_state.df, current_age_range)
                ...
                if result is not None:
                    ...
                    st.session_state.X_train_scaled = result['X_train_scaled']
                    st.session_state.X_test_scaled = result['X_test_scaled']
                    ...
                    st.session_state.label_encoders = result['label_encoders']
                    st.session_state.cv_scores = result['cv_scores']
                    st.session_state.importance_df = result['importance_df']
                    st.session_state.best_model = result['best_model']
                    st.session_state.model_results = result['model_results']
                    st.session_state.models_trained = True
                    st.session_state.last_trained_age_range = current_age_range
                    st.session_state.shap_values = result.get('shap_values', {})
                    st.session_state.shap_explainers = result.get('shap_explainers', {})
                    
                    st.success(f"✅ Models auto-trained for age group {current_age_range[0]}-{current_age_range[1]} using {result['samples_used']} samples!")
                    return True
                else:
                    st.warning(f"⚠️ Could not train models for age range {current_age_range[0]}-{current_age_range[1]}. Insufficient data.")
                    st.session_state.models_trained = False
                    return False
        return True

def display_shap_analysis():
    """Display SHAP analysis for all trained models"""
    if not SHAP_AVAILABLE:
        st.warning("⚠️ SHAP library is not installed. Install it using: pip install shap")
        return
    
    if not st.session_state.models_trained:
        st.info("Please train models first to see SHAP analysis.")
        return
    ...
    - **Positive SHAP values** (red) increase hypertension risk
    - **Negative SHAP values** (blue) decrease hypertension risk
    - **Feature importance** shows which factors most influence predictions
    """)
    
    model_tabs = st.tabs(["Logistic Regression SHAP", "Random Forest SHAP", "XGBoost SHAP"])
    
    # Logistic Regression SHAP
    with model_tabs[0]:
        if "Logistic Regression" in st.session_state.shap_values and st.session_state.shap_values["Logistic Regression"] is not None:
            st.markdown("#### Global Interpretability: Logistic Regression")
            st.markdown("Summary plot showing feature impacts across all patients in the test set")
            
            shap_values_lr = st.session_state.shap_values["Logistic Regression"]
            X_test_curr = st.session_state.X_test_scaled
            
            if X_test_curr is not None and len(X_test_curr) > 0 and shap_values_lr is not None:
                try:
                    # Create summary plot
                    fig, ax = plt.subplots(figsize=(10, 6))
                    shap.summary_plot(shap_values_lr, X_test_curr, feature_names=st.session_state.feature_names, show=False)
                    plt.title(f"SHAP Summary Plot - Logistic Regression (Age {st.session_state.last_trained_age_range[0]}-{st.session_state.last_trained_age_range[1]})", fontsize=14, weight='bold')
                    plt.tight_layout()
                    st.pyplot(fig)
                    plt.close()
                    
                    # Bar plot for mean absolute SHAP values
                    st.markdown("#### Mean Absolute SHAP Values (Global Feature Importance)")
                    fig2, ax2 = plt.subplots(figsize=(10, 6))
                    shap.summary_plot(shap_values_lr, X_test_curr, feature_names=st.session_state.feature_names, plot_type="bar", show=False)
                    plt.title(f"Feature Importance - Logistic Regression", fontsize=14, weight='bold')
                    plt.tight_layout()
                    st.pyplot(fig2)
                    plt.close()
                except Exception as e:
                    st.error(f"Error plotting SHAP for Logistic Regression: {str(e)}")
            else:
                st.warning("Test data not available for SHAP analysis")
        else:
            st.info("SHAP values not available for Logistic Regression. Please retrain models.")
    
    # Random Forest SHAP
    with model_tabs[1]:
        if "Random Forest" in st.session_state.shap_values and st.session_state.shap_values["Random Forest"] is not None:
            st.markdown("#### Global Interpretability: Random Forest")
            st.markdown("Beeswarm plot showing feature impacts across all patients")
            
            shap_values_rf = st.session_state.shap_values["Random Forest"]
            X_test_curr = st.session_state.X_test
            
            if X_test_curr is not None and len(X_test_curr) > 0 and shap_values_rf is not None:
                try:
                    # Ensure shap_values has correct shape
                    if len(shap_values_rf.shape) == 3:
                        shap_values_rf = shap_values_rf[:, :, 1]
                    
                    # Create beeswarm plot
                    fig, ax = plt.subplots(figsize=(12, 8))
                    shap.summary_plot(shap_values_rf, X_test_curr, feature_names=st.session_state.feature_names, show=False, max_display=len(st.session_state.feature_names))
                    plt.title(f"SHAP Beeswarm Plot - Random Forest (All {len(st.session_state.feature_names)} Features)", fontsize=14, weight='bold')
                    plt.tight_layout()
                    st.pyplot(fig)
                    plt.close()
                    
                    # Bar plot
                    st.markdown("#### Mean Absolute SHAP Values (Global Importance)")
                    fig2, ax2 = plt.subplots(figsize=(10, 6))
                    shap.summary_plot(shap_values_rf, X_test_curr, feature_names=st.session_state.feature_names, plot_type="bar", show=False, max_display=len(st.session_state.feature_names))
                    plt.title(f"Random Forest Feature Importance (All Attributes)", fontsize=14, weight='bold')
                    plt.tight_layout()
                    st.pyplot(fig2)
                    plt.close()
                except Exception as e:
                    st.error(f"Error plotting SHAP for Random Forest: {str(e)}")
                    st.info("Try retraining models or check if you have sufficient data.")
            else:
                st.warning("Test data not available for SHAP analysis")
        else:
            st.info("SHAP values not available for Random Forest. Please retrain models.")
    
    # XGBoost SHAP
    with model_tabs[2]:
        if XGB_AVAILABLE and "XGBoost" in st.session_state.shap_values and st.session_state.shap_values["XGBoost"] is not None:
            st.markdown("#### Global Interpretability: XGBoost")
            st.markdown("Beeswarm plot showing feature impacts across all patients")
            
            shap_values_xgb = st.session_state.shap_values["XGBoost"]
            X_test_curr = st.session_state.X_test
            
            if X_test_curr is not None and len(X_test_curr) > 0 and shap_values_xgb is not None:
                try:
                    # Ensure shap_values has correct shape
                    ...
                    # Create beeswarm plot
                    fig, ax = plt.subplots(figsize=(12, 8))
                    shap.summary_plot(shap_values_xgb, X_test_curr, feature_names=st.session_state.feature_names, show=False, max_display=len(st.session_state.feature_names))
                    plt.title(f"SHAP Beeswarm Plot - XGBoost (All {len(st.session_state.feature_names)} Features)", fontsize=14, weight='bold')
                    plt.tight_layout()
                    st.pyplot(fig)
                    plt.close()
                    ...
                    st.markdown("#### Mean Absolute SHAP Values (Global Importance)")
                    fig2, ax2 = plt.subplots(figsize=(10, 6))
                    shap.summary_plot(shap_values_xgb, X_test_curr, feature_names=st.session_state.feature_names, plot_type="bar", show=False, max_display=len(st.session_state.feature_names))
                    plt.title(f"XGBoost Feature Importance (All Attributes)", fontsize=14, weight='bold')
                    plt.tight_layout()
                    st.pyplot(fig2)
                    plt.close()
                except Exception as e:
                    st.error(f"Error plotting SHAP for XGBoost: {str(e)}")
            else:
                st.warning("Test data not available for SHAP analysis")
        elif not XGB_AVAILABLE:
            st.info("XGBoost not installed. Install it to see XGBoost SHAP analysis.")
        else:
            st.info("SHAP values not available for XGBoost. Please retrain models.")
    
    # SHAP Interpretation Guidelines
    with st.expander("📖 How to Interpret SHAP Analysis"):
        st.markdown("""
        ### Understanding SHAP Values
        
        **What are SHAP values?**
        - SHAP (SHapley Additive exPlanations) values show how much each feature contributes to a prediction
        - Based on game theory - each feature gets a "fair share" of the prediction
        
        **Reading the Summary Plot (Beeswarm):**
        - **X-axis**: SHAP value (impact on prediction)
          - Positive values (right side) → increases hypertension risk
          - Negative values (left side) → decreases hypertension risk
        ...
        **Reading the Bar Plot:**
        ...
        - Indicates overall feature importance across all predictions
        - Higher bars = more influential features
        
        **Key Insights from SHAP Analysis:**
        1. **Most important features**: Look for features with highest |SHAP| values
        2. **Direction of impact**: Check if high values of a feature increase or decrease risk
        3. **Non-linear relationships**: See how feature values affect predictions across the range
        
        **Clinical Applications:**
        - Identify modifiable risk factors (e.g., salt intake, stress, sleep)
        - Understand population-specific risk patterns
        - Provide personalized recommendations based on key drivers
...
def display_risk_results(risk_score, risk_category, color, recommendations, 
    ...
    if ml_based and model_name:
        st.info(f"✅ Using {model_name} ML model trained on your dataset (Age range: {st.session_state.last_trained_age_range[0]}-{st.session_state.last_trained_age_range[1]} years)")
    elif ml_based:
...
with st.sidebar:
    ...
    if st.session_state.df is not None:
        ...
        # Age filter - store in session state
        age_min = int(st.session_state.df['Age'].min())
        age_max = int(st.session_state.df['Age'].max())
        
        # Store previous age range to detect changes
        previous_age_range = st.session_state.age_range
        ...
        age_range = st.slider(
            ...
            help="Filter data by age range. Models will automatically retrain when this changes."
        )
        
        # Check if age range changed
        if age_range != previous_age_range:
            st.session_state.age_range = age_range
            # Mark models as not trained for this new age range
            st.session_state.models_trained = False
            st.info(f"🔄 Age range changed to {age_range[0]}-{age_range[1]}. Models will retrain automatically.")
        ...
        hypertension_filter = st.multiselect(
            ...
            default=st.session_state.hypertension_filter
        )
        st.session_state.hypertension_filter = hypertension_filter
        ...
        chart_theme = st.selectbox(
            ...
            index=["plotly", "plotly_white", "plotly_dark", "ggplot2", "seaborn"].index(st.session_state.chart_theme)
        )
...
if st.session_state.df is not None:
    st.session_state.df_filtered = apply_filters(
        ...
        st.session_state.hypertension_filter
    )
else:
    ...
    if uploaded_file is not None:
        try:
            ...
            if is_valid:
                ...
                # Reset age range to default based on data
                st.session_state.age_range = (max(18, int(df_processed['Age'].min())), min(35, int(df_processed['Age'].max())))
        ...
        <p>You can download a sample dataset to see the required format:</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Read the actual CSV file from the local directory
        csv_file_path = "hypertension_dataset_meds.csv"
        
        if os.path.exists(csv_file_path):
            # Read the CSV to show a preview
            sample_df_preview = pd.read_csv(csv_file_path)
            
            with st.expander("📊 Sample Dataset Preview"):
                st.dataframe(sample_df_preview.head(10), use_container_width=True)
                st.caption(f"Total records in sample: {len(sample_df_preview)} | Columns: {', '.join(sample_df_preview.columns)}")
            
            # Provide download button for the sample dataset
            with open(csv_file_path, 'rb') as file:
                csv_data = file.read()
            
            st.download_button(
                label="📥 Download Sample Dataset (hypertension_dataset_meds.csv)",
                data=csv_data,
                file_name="hypertension_dataset_meds.csv",
                mime="text/csv",
                help="Download the complete hypertension dataset with BP_History and Medication columns"
            )
            
            st.info("💡 **Tip:** This dataset includes BP_History and Medication columns which will be automatically excluded from model training to prevent data leakage. The models will only use lifestyle factors (Age, Salt_Intake, Stress_Score, Sleep_Duration, BMI, Family_History, Exercise_Level, Smoking_Status).")
        else:
            st.warning("Sample dataset file 'hypertension_dataset_meds.csv' not found in the current directory.")
            
            # Fallback to creating a smaller sample template
            sample_data = {
                'Age': [35, 42, 28, 55, 31],
            ...
            st.download_button(
                label="📥 Download Mini Sample Template",
                data=csv,
                file_name="hypertension_mini_sample.csv",
                mime="text/csv"
            )
...
# Exploratory Analysis Page
    ...
    st.markdown(f"**Age bins: {st.session_state.bin_size}-year intervals**")
    ...
    bins, age_labels = create_age_bins(age_min_filtered, age_max_filtered, st.session_state.bin_size)
    ...
    with analysis_tabs[6]:
        st.markdown("### Advanced Age Analysis")
        st.markdown("""
        This section provides deeper insights into how hypertension prevalence changes with age.
        It includes overall age distribution, hypertension trend analysis, and identification of 
        the age at which 50% of the population develops hypertension.
        """)
        
        # Convert hypertension to numeric for calculations
        df_for_chart['Hypertension_Num'] = df_for_chart['Has_Hypertension'].map({'Yes': 1, 'No': 0})
        
        # 1. Overall Age Distribution (Histogram with KDE)
        st.markdown("#### Overall Age Distribution of Respondents")
        ...
        # Add KDE line (using scipy if available, otherwise skip)
        try:
            from scipy import stats
            # Filter out NaN values
            age_clean = df_for_chart['Age'].dropna()
            if len(age_clean) > 3:  # Need at least a few points for KDE
                kde = stats.gaussian_kde(age_clean)
                x_range = np.linspace(age_clean.min(), age_clean.max(), 100)
                y_density = kde(x_range) * len(age_clean) * (age_clean.max() - age_clean.min()) / 20
                ...
                fig_age_dist.add_trace(go.Scatter(
                    ...
                    hovertemplate='Age: %{x:.1f}<br>Density: %{y:.1f}<extra></extra>'
                ))
        ...
        fig_age_dist.update_layout(
            title='Overall Age Distribution of Respondents',
        ...
        st.markdown("#### Age Summary Statistics")
        age_clean = df_for_chart['Age'].dropna()
        if len(age_clean) > 0:
            age_stats = age_clean.describe()
            age_stats_df = pd.DataFrame({
                'Statistic': ['Count', 'Mean', 'Std Dev', 'Min', '25th Percentile', 'Median', '75th Percentile', 'Max'],
                'Value': [age_stats['count'], f"{age_stats['mean']:.1f}", f"{age_stats['std']:.1f}", 
                         age_stats['min'], age_stats['25%'], age_stats['50%'], age_stats['75%'], age_stats['max']]
            })
            st.dataframe(age_stats_df, use_container_width=True)
        else:
            st.warning("No valid age data available.")
        
        # 2. Hypertension Trend Analysis
        st.markdown("#### Hypertension Trend by Age Group")
        
        # Categorize age into decades
        def categorize_age_decade(age):
            if age < 20: return '<20'
            elif 20 <= age < 30: return '20-29'
            elif 30 <= age < 40: return '30-39'
            elif 40 <= age < 50: return '40-49'
            elif 50 <= age < 60: return '50-59'
            elif 60 <= age < 70: return '60-69'
            elif 70 <= age < 80: return '70-79'
            else: return '80+'
        
        df_for_chart['Age_Decade'] = df_for_chart['Age'].apply(categorize_age_decade)
        decade_order = ['<20', '20-29', '30-39', '40-49', '50-59', '60-69', '70-79', '80+']
        
        # Calculate hypertension rate per decade
        hypertension_by_decade = df_for_chart.groupby('Age_Decade')['Hypertension_Num'].mean().reindex(decade_order) * 100
        x_vals = np.arange(len(decade_order))
        y_vals = hypertension_by_decade.values
        ...
        fig_trend.add_trace(go.Scatter(
            ...
            marker=dict(size=12, color='#FF4C4C'),
            hovertemplate='Age Group: %{text}<br>Rate: %{y:.1f}%<extra></extra>',
            text=decade_order
        ))
        ...
        # Add trend line (only if we have valid non-NaN values and enough variation)
        valid_mask = ~np.isnan(y_vals)
        x_valid = x_vals[valid_mask]
        y_valid = y_vals[valid_mask]
        
        if len(x_valid) >= 2 and len(np.unique(y_valid)) > 1:
            try:
        ...
        fig_trend.add_hline(y=50, line_dash="dot", line_color="#2C3E50", 
                           annotation_text="50% Risk Threshold", 
                           annotation_position="bottom right")
        
        # Calculate exact age at 50% risk
        def estimate_age_for_50_percent(decades, rates):
            decade_midpoints = []
            for d in decades:
                if d == '<20': decade_midpoints.append(15)
                elif d == '80+': decade_midpoints.append(85)
                else:
                    try:
                        a, b = d.split('-')
                        decade_midpoints.append((int(a) + int(b)) / 2)
                    except:
                        decade_midpoints.append(50)  # fallback
            
            decade_midpoints = np.array(decade_midpoints)
            
            for i in range(len(rates) - 1):
                y1, y2 = rates[i], rates[i + 1]
                x1, x2 = decade_midpoints[i], decade_midpoints[i + 1]
                if not np.isnan(y1) and not np.isnan(y2):
                    if (y1 <= 50 <= y2) or (y2 <= 50 <= y1):
                        age_50 = x1 + (50 - y1) * (x2 - x1) / (y2 - y1)
                        return age_50, (decades[i], decades[i+1])
            return None, None
        
        age_at_50, crossing_decades = estimate_age_for_50_percent(decade_order, y_vals)
        
        if age_at_50 and crossing_decades:
            # Find x-coordinate for annotation
            try:
                idx_start = decade_order.index(crossing_decades[0])
                if idx_start + 1 < len(y_vals) and not np.isnan(y_vals[idx_start]) and not np.isnan(y_vals[idx_start + 1]):
                    x_intercept = idx_start + (50 - y_vals[idx_start]) / (y_vals[idx_start+1] - y_vals[idx_start])
                    
                    fig_trend.add_trace(go.Scatter(
                        x=[x_intercept], y=[50],
                        mode='markers',
                        marker=dict(size=15, color='blue', symbol='circle'),
                        name=f'50% Risk at age ~{age_at_50:.1f}',
                        hovertemplate=f'Age: {age_at_50:.1f} years<br>Risk: 50%<extra></extra>'
                    ))
                    
                    fig_trend.add_annotation(
                        x=x_intercept, y=45,
                        text=f"Risk hits 50% at age ~{age_at_50:.1f}",
                        showarrow=True,
                        arrowhead=2,
                        arrowsize=1,
                        arrowwidth=2,
                ...
                pass  # Skip annotation if there's an indexing issue
        
        fig_trend.update_layout(
            title='Hypertension Trend by Age Decade',
            xaxis_title='Age Group',
            yaxis_title='Hypertension Rate (%)',
            template=chart_theme,
            height=500,
            xaxis=dict(tickmode='array', tickvals=x_vals, ticktext=decade_order)
        )
        st.plotly_chart(fig_trend, use_container_width=True)
        
        # 3. Side-by-side comparison of hypertension status by age group
        st.markdown("#### Comparison of Hypertension Status by Age Group")
        
        # Group data to get counts for each category
        grouped = df_for_chart.groupby(['Age_Decade', 'Has_Hypertension']).size().unstack(fill_value=0)
        grouped = grouped.reindex(decade_order)
        
        fig_comparison = go.Figure()
        
        fig_comparison.add_trace(go.Bar(
            x=x_vals - 0.2, y=grouped['No'] if 'No' in grouped.columns else [0]*len(decade_order),
            name='No Hypertension',
            marker_color='#3498DB',
            width=0.4,
            hovertemplate='Age Group: %{text}<br>Count: %{y}<extra></extra>',
            text=decade_order
        ))
        
        fig_comparison.add_trace(go.Bar(
            x=x_vals + 0.2, y=grouped['Yes'] if 'Yes' in grouped.columns else [0]*len(decade_order),
        ...
        fig_comparison.update_layout(
            title='Comparison of Hypertension Status by Age Group',
            xaxis_title='Age Group',
        ...
        # Display the calculated 50% risk age if found
        if age_at_50 and crossing_decades:
            st.success(f"📊 **Key Finding:** The population reaches 50% hypertension risk at approximately **{age_at_50:.1f} years** of age, crossing between the {crossing_decades[0]} and {crossing_decades[1]} age groups.")
        else:
            st.info("📊 The population in this dataset has not yet reached 50% hypertension risk within the observed age range, or there is insufficient data for this calculation.")
...
# Predictive Models Page
    ...
    if not st.session_state.models_trained:
        st.warning("⚠️ Could not train models with current age range. Please adjust the age range or check your data.")
        st.stop()
    ...
    <br><strong><span style="color: #2ecc71;">✓ Current Training Age Range:</span></strong> <strong>{st.session_state.last_trained_age_range[0]} - {st.session_state.last_trained_age_range[1]} years</strong>
    <br><strong>Note:</strong> Changing the age range in the sidebar will automatically retrain models.
    </div>
    """, unsafe_allow_html=True)
    
    # Create tabs for different model analyses
    predictive_tabs = st.tabs(["📊 Model Performance", "🧠 SHAP Analysis", "📈 Learning Curves", "📉 Confusion Matrices", "⭐ Feature Importance"])
    
    # Tab 1: Model Performance
    with predictive_tabs[0]:
                ...
                if X_curr is not None and len(X_curr) > 0:
                    ...
                    if fig is not None:
                        ...
                        **Learning Curve Interpretation for {name}:**
                        ...
                        - Shaded areas: ±1 standard deviation across CV folds
                        """)
                    else:
    ...
    with insights_col1:
        ...
        <ul>
        <li>Models trained on ages {st.session_state.last_trained_age_range[0]}-{st.session_state.last_trained_age_range[1]} from your dataset</li>
...
# Risk Assessment Page
    ...
    if not st.session_state.models_trained:
        ...
        <p>Could not train models with the current age range. Please adjust the age range or check your data.</p>
        ...
        <p><strong><span style="color: #2ecc71;">✓ Training Age Range:</span></strong> {st.session_state.last_trained_age_range[0]} - {st.session_state.last_trained_age_range[1]} years</p>
        <p><strong>Note:</strong> Changing the age range in the sidebar will automatically retrain models.</p>
        </div>
        """, unsafe_allow_html=True)
        use_ml_model = True
    ...
    with st.form("risk_assessment_form"):
        ...
        # BMI Information Box
        with st.expander("📏 How to Calculate BMI (Body Mass Index)", expanded=False):
            st.markdown("""
            ...
            **Quick Reference (Height in cm):**
            | Height | Normal Weight Range | Overweight Starts |
            |--------|-------------------|------------------|
            | 150 cm | 42-56 kg | 56 kg |
            | 160 cm | 47-64 kg | 64 kg |
            | 170 cm | 53-72 kg | 72 kg |
            | 180 cm | 60-81 kg | 81 kg |
            | 190 cm | 67-90 kg | 90 kg |
            """)
        ...
        with col1:
            ...
            age = st.number_input("Age (years)", min_value=1, max_value=120, value=30, step=1,
                                 help="Age is a key risk factor - risk increases with age")
            family_history = st.radio("Family History of Hypertension", ["Yes", "No"], horizontal=True,
                                     help="Having a parent or sibling with hypertension increases your risk")
        
        with col2:
            st.markdown("#### Health Measurements")
            bmi = st.number_input("Body Mass Index (BMI)", min_value=10.0, max_value=60.0, value=25.0, step=0.1,
                                 help="BMI = weight(kg) / height(m)². Click 'How to Calculate BMI' above for guidance")
            st.caption("💡 Click the expander above to learn how to calculate your BMI")
        
        with col3:
            st.markdown("#### Lifestyle Factors")
            
            # Exercise Level with detailed info
            exercise_options = ["Low", "Moderate", "High"]
            exercise_help = """
            **Low:** Sedentary lifestyle, little to no exercise (e.g., sit at desk all day, no physical activity)
            **Moderate:** Regular walking, light sports, 30 min activity 3-5 days/week (e.g., brisk walking, cycling, yoga)
            **High:** Intense daily exercise, athletic training (e.g., running, swimming, gym workouts, sports)
            """
            exercise_level = st.select_slider("Exercise Level", options=exercise_options, help=exercise_help)
            
            # Smoking Status
            smoking_status = st.radio("Smoking Status", ["Non-Smoker", "Smoker"], horizontal=True,
                                     help="Smoking significantly increases hypertension and cardiovascular risk")
            
            # Salt Intake with detailed info
            salt_help = """
            **Recommended daily intake:** <5-6g per day (about 1 teaspoon)
            
            **Salt content in common foods:**
            - 1 slice of bread: 0.2-0.3g
            - 1 slice of pizza: 1.5-2.5g
            - Fast food meal: 3-5g
            - Instant noodles: 3-5g per serving
            - Processed meat (100g): 1.5-2.5g
            - Canned soup (1 cup): 0.7-1.2g
            ...
            **Tips to reduce salt:**
            ...
            # Stress Score with detailed info
            stress_help = """
            **Stress Scale Guide (0-10):**
            - **0-2:** Minimal stress, very relaxed
            - **3-4:** Mild stress, occasional worry
            - **5-6:** Moderate stress, noticeable tension
            - **7-8:** High stress, frequent anxiety
            - **9-10:** Severe stress, overwhelming pressure
            ...
            **Common stress indicators:**
            ...
            - Fatigue
            """
            stress_score = st.slider("Stress Level (0-10)", 0, 10, 5, help=stress_help)
            
            # Sleep Duration with detailed info
            sleep_help = """
            ...
            **Health impacts:**
            - **<6 hours:** Increased cardiovascular risk, impaired immunity
            - **6-7 hours:** Suboptimal but acceptable
            - **7-9 hours:** Optimal for health
            - **>9 hours:** May indicate underlying health issues
            
            **Tips for better sleep:**
            - Maintain consistent sleep schedule
            - Avoid screens before bedtime
            - Create dark, quiet sleeping environment
...
# About Page
    ...
    with col1:
        ...
        4. **Predictive Modeling**: Real machine learning models trained on your data with SHAP analysis
        ...
        - **Logistic Regression**: Linear SHAP explainer showing coefficient-based importance
        - **Random Forest**: Tree SHAP explainer showing non-linear feature interactions
        - **XGBoost**: Optimized Tree SHAP explainer with advanced feature impact detection
        - **Global Interpretability**: Summary plots and bar charts for overall feature importance
        - **Local Interpretability**: Waterfall plots for individual patient predictions (Logistic Regression only)
        
        #### Automatic Model Retraining:
        
        - Models automatically retrain when age range changes
        - Real-time adaptation to your selected population
        - Ensures risk assessment always uses the most relevant data
        ...
        5. **Risk Calculation**: ML-based prediction using the best performing model
    ...
    with col2:
        ...
        3. **XGBoost Classifier** (100 estimators, if available)
        ...
        - Minimum Data: 10 records recommended per age group
    ...
    if os.path.exists(csv_file_path):
        ...
        st.download_button(
            label="📥 Download Complete Hypertension Dataset (with BP_History and Medication)",
            data=csv_data,
            file_name="hypertension_dataset_meds.csv",
            mime="text/csv",
            help="Download the full dataset containing BP_History and Medication columns"
        )
        
        st.info("💡 **Note:** This dataset includes BP_History and Medication columns which will be automatically excluded from model training to prevent data leakage.")
    else:
        st.warning("Sample dataset file 'hypertension_dataset_meds.csv' not found in the current directory.")
        
        # Fallback to creating a smaller sample
        sample_data = {
        ...
        st.download_button(
            label="📥 Download Sample Dataset (10 records)",
            data=csv,
...
with footer_col1:
    ...
    st.markdown("Version 8.3 | April 2026")
...
with footer_col3:
    st.markdown("**SHAP-Powered Insights**")
